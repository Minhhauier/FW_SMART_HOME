...
void reopen_network()
{
    snprintf(buffer, 128, "Smart Home Device");
    ...
    ESP_LOGI(TAG, "AP mode re-enabled. SSID: %s", buffer);
}
...
static void wifi_init_softap(void)
{
    ...
    ESP_LOGI(TAG, " WiFi AP started. SSID: %s ", buffer);
}
...
//   scan_wifi_to_connect();
    ...
    ESP_LOGI(TAG, " Web server started! Connect to WiFi AP 'Smart home device' and open http://192.168.4.1/");
}